...
module.exports = function(buffer) {
    var key = buffer.id;
    console.log('requeted analyzer for', key);
    if (buffers.has(key)) {
...
function markBufferWithAnalyzerData(buffer, data) {
    // first, find and remove previous LiveStyle markers
    // TODO update existng markers instead of removing?
    buffer.getMarkers(marker => {
        if ('_livestyle' in marker) {
            marker.destroy();
        }
    });

    // add new markers
    const markerProps = {persistent: false};
    convert(data).source.all().forEach(node => {
        // console.log('analysis node', node);
        if (!node.fullRange || !node.fullRange.length) {
            return;
        }

        let startPos = buffer.positionForCharacterIndex(node.fullRange[0]);
        let endPos = buffer.positionForCharacterIndex(node.fullRange[1]);

        let marker = buffer.markRange([startPos, endPos], markerProps);
        marker._livestyle = {
