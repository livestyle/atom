...
function markBufferWithAnalyzerData(buffer, data) {
    ...
    let newMarkers = new Set(data.source.all()
...
function createMarkerData(node, buffer) {
    ...
    if (node.type === 'root') {
        ...
        range = valueRange;
    } else if (node.type === 'section' && node.analysis.selector && node.analysis.selector !== node.name) {
        range = nameRange;
    }
