...
function markBufferWithAnalyzerData(buffer, data) {
    ...
    .map(node => {
        ...
        return {
            ...
            range: node.type === 'root'
                ? [[0, 0], [0, 0]]
                : utils.makePositionRange(buffer, node.fullRange),
            nameRange: utils.makePositionRange(buffer, node.nameRange),
            valueRange: utils.makePositionRange(buffer, node.valueRange),
            type: node.type,
            livestyle: {
                info: node.analysis,
                type: node.type,
