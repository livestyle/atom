    ...
    newMarkers.forEach(item => {
        ...
        if (matched.length) {
            ...
            matched.forEach(marker => {
                // FIXME LiveStyle data contains circular references, which
                // leads to max call stack error
                let isEqual = false;
                try {
                    isEqual = deepEqual(marker.getProperties().livestyle, item.livestyle);
                } catch (e) {
                    console.log('Got error while comparing %o and %o', marker.getProperties().livestyle, item.livestyle);
                    throw e;
                }
...
function createMarkerData(node, buffer) {
    ...
    if (node.type === 'root') {
        ...
        range = [[0, 0], [0, 0]];
    } else if (node.analysis.computed != null) {
        range = valueRange;
    }
