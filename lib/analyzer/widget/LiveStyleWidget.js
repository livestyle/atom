...
class LiveStyleWidget extends HTMLElement {
    initialize(editor, editorView, marker, options) {
        console.log('created Livestyle Widget');
        this.editor = editor;
    ...
    update() {
        let topLeft = this.editorView.pixelPositionForBufferPosition(this.marker.getStartBufferPosition());
        let bottomRight = this.editorView.pixelPositionForBufferPosition(this.marker.getEndBufferPosition());
        let scroll = {
    ...
    setupEvents() {
        ...
        subscriptions.add(
            ...
            this.marker.onDidChange(update)
        );
